...
| Model program | `CudaExecutor<P>` owns lowering into reusable host buffers, completion, output validation, and sampling; the Llama adapter constructs a validated model-neutral dense-decoder artifact; graph policy and the concrete flat-KV attention implementation are separate private modules | preserve this composition when adding the second dense architecture and attention backend, then move stable device workspaces into the executor |
